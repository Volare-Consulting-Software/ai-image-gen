...
// the image was tagged as having a white/black background. SVG requires the
// image to be tagged shape-available.
export async function GET(
  ...
  if (format === "svg" && !image.shapeAvailable) {
    return NextResponse.json({ error: "SVG is not available for this image" }, { status: 400 });
  }
